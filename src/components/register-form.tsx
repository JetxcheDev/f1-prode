...
interface RegisterFormProps extends React.ComponentProps<"div"> {
  onEmailRegister: (values: z.infer<typeof RegisterSchema>) => Promise<{ success?: string; error?: string }>;
  onGoogleRegister: () => Promise<{ success?: string; error?: string }>;
}
...
export function RegisterForm({
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      const errorMessage = result.error.issues[0]?.message || "Por favor, completa todos los campos correctamente";
      toast.error(errorMessage);
      return;
    }

    startTransition(async () => {
      const response = await onEmailRegister({ name, email, password, confirmPassword });
      if (response.success) {
  ...
  const handleGoogleRegister = () => {
    startTransition(async () => {
      const response = await onGoogleRegister();
      if (response.success) {
        toast.success(response.success);
      } else if (response.error) {
        toast.error(response.error);
          ...
          <form onSubmit={handleEmailSubmit} className="p-6 md:p-8">
            <div className="flex flex-col gap-6">
              <div className="flex flex-col items-center text-center">
                ...
                <h1 className="text-2xl font-bold">Únete a F1 Prode</h1>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center text-white">
                ...
                <p className="text-lg opacity-90 max-w-sm mx-auto">
                  Únete a la comunidad de fanáticos de F1 y demuestra tus conocimientos
                </p>
              </div>
            </div>
